...
module.exports.run = async (client, message, args, { guild, user, error }) => {
	...
	if (!memberMention.bannable) return message.channel.send(
		new RichEmbed().setTitle("Softban").setColor(Colors.FAILED).setDescription("I cannot softban the mentioned user. Make sure the Toast role is above the other roles.").setFooter(message.author.tag, message.author.displayAvatarURL)
	);
...
module.exports.data = {
	name: "softban",
	description: "Bans the specified user for the specified amount of days",
	type: "mod",
